...
class Affix_Type
{
  public:
    ...
    const char *stringify() {
        return Perl_form_nocontext("%s%s%s%s%s", (pointer_flag ? "Pointer[ " : ""),
                                   (const_flag ? "Const[ " : ""), _stringify,
                                   (const_flag ? " ]" : ""), (pointer_flag ? " ]" : "")

        );
    };
    size_t alignment() { return pointer_flag ? ALIGNOF_INTPTR_T : _alignment; }
