...
my $wrap_w_context = wrap( find_library('m'), 'pow', undef,              Double );
affix( find_library('m'), [ 'pow' => 'affix_w_params' ],  [ Double, Double ], Double );
affix( find_library('m'), [ 'pow' => 'affix_w_context' ], undef,              Double );
#
subtest verify => sub {
...
subtest benchmarks => sub {
    ...
    subtest 'known args' => sub {
        isnt fastest( -5, wrap => sub { $wrap_w_params->( 3, 4 ) }, affix => sub { affix_w_params( 3, 4 ) }, pure => sub { pow( 3, 4 ) } ), 'pure',
            '[Int, Int]';
        isnt fastest( -5, wrap => sub { $wrap_w_params->( 3.0, 4.0 ) }, affix => sub { affix_w_params( 3.0, 4.0 ) },
            pure => sub { pow( 3.0, 4.0 ) } ), 'pure', '[Double, Double]';
    };
    subtest 'rely on context' => sub {
        isnt fastest( -5, wrap => sub { $wrap_w_context->( 3, 4 ) }, affix => sub { affix_w_context( 3, 4 ) }, pure => sub { pow( 3, 4 ) } ), 'pure',
            '[Int, Int]';
        isnt fastest(
            -5,
            wrap  => sub { $wrap_w_context->( 3.0, 4.0 ) },
            affix => sub { affix_w_context( 3.0, 4.0 ) },
            pure  => sub { pow( 3.0, 4.0 ) }
            ),
            'pure', '[Double, Double]';
    };
};
