...
<tr> <td>SSize_t</td> <td>SSize_t</td> <td>SSize_t</td> <td></td> <td></td>       <td></td>     </tr>
...
A value where C<ref($value)> equals C<CODE>. This would be how callbacks are defined.

The argument list and return value must be defined. For example, C<Callback[[Int, Int]=>Int]> ~~ C<typedef int
(*fuc)(int a, int b);>; that is to say our function accepts two integers and returns an integer.

    Callback[[] => Void];                   # typedef void (*function)();
    Callback[[Pointer[Int]] => Int];        # typedef Int (*function)(int * a);
    Callback[[Str, Int] => Struct[...]];    # typedef struct Person (*function)(chat * name, int age);
